import { addDays, isCalendarDate } from '@/lib/quote/dates';
 ...
 *
 * The check belongs beside `addDays` in the quote engine -- that is the
 * function it protects, and the quote engine cannot import from the schedule
 * to validate a date. `readAnchor` and this file's own `epochDay` found it
 * here first, so the name stays available here.
 */
